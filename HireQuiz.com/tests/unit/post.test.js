...
describe('POST /v1/posting', () => {
  test('unauthenticated user should receive 401 error', () =>
    request(app).post('/v1/posting').expect(401));

  test('incorrect credentials should receive 401 error', () =>
    request(app).post('/v1/posting').auth('invalid@email.com', 'incorrect_password').expect(401));
  ...
  test('check the data in array after post request is done', async () => {
    ...
    expect(data.posting.ownerId).toBe(userID);
